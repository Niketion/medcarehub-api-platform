...
public sealed class SlotsControllerTests
{
    ...
    [Fact]
    public async Task Create_ShouldReturnConflict_WhenDoctorHasOverlappingSlot()
    {
        await using var db = TestDbFactory.Create();

        var existing = new Slot
        {
            Id = Guid.NewGuid(),
            DoctorId = "dr-rossi",
            StartsAt = new DateTimeOffset(2026, 04, 10, 9, 0, 0, TimeSpan.Zero),
            EndsAt = new DateTimeOffset(2026, 04, 10, 10, 0, 0, TimeSpan.Zero),
            Status = SlotStatus.Available
        };

        db.Slots.Add(existing);
        await db.SaveChangesAsync();
        ...
        var result = await sut.Create(
            new CreateSlotRequest(
                "dr-rossi",
                null,
                new DateTimeOffset(2026, 04, 10, 9, 30, 0, TimeSpan.Zero),
                new DateTimeOffset(2026, 04, 10, 10, 30, 0, TimeSpan.Zero)),
            CancellationToken.None);
